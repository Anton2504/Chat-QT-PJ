...
QByteArray Database::isAdmin(const QString& login){
    QByteArray adminUuid;
    if (login == "Admin"){
        QSqlQuery query;
        query.prepare("select uuid from users where login = 'Admin' and status = '\x28'");
        if(!query.exec()){
            qDebug() << "Error inserting record:" << query.lastError().text();
        }else{
            if(query.next()){
                adminUuid = query.value("uuid").toByteArray();
                return adminUuid;
            }
        }
    }
...
bool Database::getDbUSers(){
    ...
    query.prepare("select login, uuid from users where status = 0x01");
    if(!query.exec()){
        qDebug() << "Error select login, uuid from users:" << query.lastError().text();
        return false;
    }else{
...
bool Database::newAllMsg(const QString& login, const QString& msg){
    ...
    QSqlQuery query;
    QByteArray sender;
    login != "Admin"?sender = _users.value(login):sender = isAdmin(login);
    ...
    query.bindValue(":uuid_sender", sender);
...
bool Database::newPrivateMsg(const QString& sender, const QString& recipient, const QString& msg){
    ...
    getDbUSers();
    QSqlQuery query;
    QByteArray uuid_sender;
    sender != "Admin"?uuid_sender = _users.value(sender):uuid_sender = isAdmin(sender);
    QUuid uuidMsg = QUuid::createUuidV7();
    ...
    query.bindValue(":uuid_msg", uuidMsg);
    query.bindValue(":uuid_sender", uuid_sender);
...
bool Database::authLogin(const QString& login, const QString& pass){
    ...
    if(!loginStatus(login).isEmpty()){
        if( loginStatus(login).at(0) != 0x00 &&
            loginStatus(login).at(0) != 0x02 &&
            loginStatus(login).at(0) != 0x14 &&
            chekPass(login,pass))
            return true;
    }
    return false;
...
QByteArray Database::loginStatus (const QString& login){
    QByteArray status(1,0x00);
    QSqlQuery query;
    query.prepare("select status from users where login = :login ");
    query.bindValue(":login", login);
    if(!query.exec()){
        qDebug() << "Error inserting record:" << query.lastError().text();
        status = QByteArray("0x14");
        return status;
    }else{
        if(query.next()){
            status = query.value("status").toByteArray();
            return status;
        }
        return status;
    }
